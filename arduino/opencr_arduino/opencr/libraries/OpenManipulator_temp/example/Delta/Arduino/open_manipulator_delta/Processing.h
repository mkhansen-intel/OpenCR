...
void sendValueToProcessing(Delta *delta)
{
  sendAngle2Processing(delta->getManipulator()->getAllActiveJointValue());
  if(delta->getPlatformFlag()) 
    sendToolData2Processing(delta->getManipulator()->getToolValue("tool"));
  else
    sendToolData2Processing(delta->getManipulator()->getToolGoalValue("tool"));
}


void fromProcessing(Delta *delta, String data)
{
  ...
  else if (cmd[0] == "joint")
  {
    ...
    delta->jointTrajectoryMove(goal_position, 1.0); // FIX TIME PARAM
  }
  ...
  {
    if (cmd[1] == "forward")
      delta->taskTrajectoryMoveToPresentPosition("tool", RM_MATH::makeVector3(0.010, 0.0, 0.0), 0.2);
    else if (cmd[1] == "back")
      delta->taskTrajectoryMoveToPresentPosition("tool", RM_MATH::makeVector3(-0.010, 0.0, 0.0), 0.2);
    else if (cmd[1] == "left")
      delta->taskTrajectoryMoveToPresentPosition("tool", RM_MATH::makeVector3(0.0, 0.010, 0.0), 0.2);
    else if (cmd[1] == "right")
      delta->taskTrajectoryMoveToPresentPosition("tool", RM_MATH::makeVector3(0.0, -0.010, 0.0), 0.2);
    else if (cmd[1] == "up")
      delta->taskTrajectoryMoveToPresentPosition("tool", RM_MATH::makeVector3(0.0, 0.0, 0.010), 0.2);
    else if (cmd[1] == "down")
      delta->taskTrajectoryMoveToPresentPosition("tool", RM_MATH::makeVector3(0.0, 0.0, -0.010), 0.2);
    else
      delta->taskTrajectoryMoveToPresentPosition("tool", RM_MATH::makeVector3(0.0, 0.0, 0.0), 0.2);
  }
  ...
  else if (cmd[0] == "get")
  {
    ...
    else if (cmd[1] == "pose")  // save pose
    {
      ...
      read_value.angle = delta->getManipulator()->getAllActiveJointValue();
      read_value.path_time = 2.0;
      read_value.gripper_value = delta->getManipulator()->getToolGoalValue("tool");
  ...
  else if (cmd[0] == "motion")
  {
    if (cmd[1] == "start")
    {
      Pose present_pose = delta->getManipulator()->getComponentPoseToWorld("tool");
...
void playProcessingMotion(Delta *delta)
{
  if(!delta->isMoving() && processing_motion_flag)
  {
    if(motion_way_point_buf.size() == 0)
      return;

    delta->toolMove("tool", motion_way_point_buf.at(hand_motion_cnt).gripper_value);
    delta->jointTrajectoryMove(motion_way_point_buf.at(hand_motion_cnt).angle, motion_way_point_buf.at(hand_motion_cnt).path_time); 
    hand_motion_cnt ++;
