...
#ifndef OPEN_MANIPULATOR_LINEAR_H_
#define OPEN_MANIPULATOR_LINEAR_H_
...
OPEN_MANIPULATOR::OpenManipulator linear;

//-- Kinematics Init --//
OPEN_MANIPULATOR::Kinematics *kinematics = new OM_KINEMATICS::Linear();
...
{
  linear.addWorld(WORLD,
                 COMP1);
  linear.addComponent(COMP1,
                     WORLD,
                     COMP4,
  ...
  linear.addTool(TOOL,    // why defined like this???
                COMP7,
                OM_MATH::makeVector3(0.0366, 0.0, 0.0),
                Eigen::Matrix3f::Identity(3, 3));

  linear.initKinematics(kinematics);
#ifdef PLATFORM ////////////////////////////////////Actuator init
  linear.initActuator(actuator);
  uint32_t baud_rate = BAUD_RATE;
  void *p_baud_rate = &baud_rate;
  linear.actuatorInit(p_baud_rate);
  linear.setActuatorControlMode2();
  linear.actuatorEnable();
#endif /////////////////////////////////////////////
  linear.initJointTrajectory();
  linear.setControlTime(ACTUATOR_CONTROL_TIME);
  ...
  goal_position_.push_back(0.0f);
  goal_position_.push_back(0.0f);
  linear.jointMove(goal_position_, 1.0f);

  linear.setAllActiveJointAngle(linear.receiveAllActuatorAngle());
#endif /////////////////////////////////////////////
