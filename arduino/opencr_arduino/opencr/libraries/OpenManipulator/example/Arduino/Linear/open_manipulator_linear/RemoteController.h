...
{
  if (data & RC100_BTN_U)
    linear.setMove(TOOL, OM_MATH::makeVector3(0.007f, 0.0, 0.0), 0.16f);
  else if (data & RC100_BTN_D)
    linear.setMove(TOOL, OM_MATH::makeVector3(-0.007f, 0.0, 0.0), 0.16f);
  else if (data & RC100_BTN_L)
    linear.setMove(TOOL, OM_MATH::makeVector3(0.0, 0.007f, 0.0), 0.16f);
  ...
  else if (data & RC100_BTN_6)
  {
    ...
    goal_position.push_back(0.0f);
    goal_position.push_back(0.0f);
    linear.jointMove(goal_position, 1.0f);
  }
}
#endif
